...
const bulkUpdate = require("./bulkUpdate.cjs");
...
class CustomTypesClient {
   ...
   */
  async bulkUpdate(operations, params) {
    const resolvedOperations = operations instanceof bulkUpdate.BulkUpdateTransaction ? operations.operations : operations;
    await this.fetch("./bulk-update", params, createPostFetchRequestInit({
  ...
  async fetch(path, params = {}, requestInit = {}) {
    ...
    switch (res.status) {
      ...
      case 403: {
        ...
        if ("hasExistingDocuments" in json && json.hasExistingDocuments) {
          throw new errors.BulkUpdateHasExistingDocumentsError("A custom type with published documents cannot be deleted. Delete all of the custom type's documents or remove the delete operation from the request before trying again.", { url, response: json });
        }
    ...
    throw new errors.InvalidAPIResponse("An invalid API response was returned", {
      url
    });
  }
}
